...
def select_date_and_click_book_button(page):
    """Wait until 8 AM, select class from tabmenu, and click Book Now."""
    try:
        # Wait until 8 AM
        wait_until_8am_with_millisecond_precision()
        
        # Select the class from the tabmenu at 8 AM
        class_selector = f"xpath=//p-tabmenu//li[contains(@class, 'ui-tabmenuitem')]//span[contains(text(), '{JOURNEY_CLASS}')]"
        class_element = page.wait_for_selector(class_selector, state="visible")
        class_element.scroll_into_view_if_needed()
        class_element.click()
        logging.info(f"Selected class '{JOURNEY_CLASS}' from tabmenu at 8 AM")
        ...
        # Click Book Now with retry if disabled

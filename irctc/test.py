...
def select_bhim_and_pay(page):
    """Execute payment steps with military precision and speed."""
    try:
        # 1. Click BHIM/UPI payment option (200ms timeout)
        page.click(
            "xpath=//div[contains(@class, 'bank-type')]//span[contains(., 'BHIM/ UPI/ USSD')]",
            timeout=200
        )

        # 2. Select Paytm-powered option (150ms timeout)
        page.click(
            "xpath=//span[contains(., 'Powered by PAYTM')]",
            timeout=150
        )

        # 3. Click Pay & Book with nuclear option
        page.evaluate("""() => {
            document.querySelector('button:has-text("Pay & Book")').click();
        }""")
                    ...
                    if availability:
                        ...
                        if select_date_and_click_book_button(page):
                            ...
                            if handle_captcha_and_proceed(page):
                                ...
                                # Add payment page handling here
                            else:

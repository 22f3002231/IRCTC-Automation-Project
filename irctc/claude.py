...
JOURNEY_TRAIN = "12101"
JOURNEY_CLASS = "1A"
UPI_ID = "8668967041@ybl"  # Add your UPI ID here
...
def select_bhim_and_pay(page):
    ...
    try:
        ...
        # Wait for payment page to load by waiting for the URL change instead of websocket
        page.wait_for_url(
            lambda url: "secure.paytmpayments.com/theia/processTransaction" in url,
            timeout=30000
        )
        logging.info("Payment gateway loaded successfully")
        
        # Handle UPI payment page
...
def handle_paytm_upi_payment(page):
    """Handle the Paytm UPI payment section after redirection."""
    try:
        # Wait for the page to stabilize after redirection
        page.wait_for_load_state("networkidle", timeout=30000)
        logging.info("Payment page loaded")
        
        # Step 1: Click the radio button for UPI payment
        radio_button_selector = "input[type='radio'][value='upi']"
        page.wait_for_selector(radio_button_selector, state="visible", timeout=10000)
        page.click(radio_button_selector)
        logging.info("Clicked UPI radio button")

        # Step 2: Wait for the UPI ID input field to appear
        vpa_field_selector = "input[placeholder='Enter VPA']"
        page.wait_for_selector(vpa_field_selector, state="visible", timeout=15000)
        
        # Clear any pre-filled value in the input field
        current_value = page.locator(vpa_field_selector).input_value()
        if current_value:
            page.fill(vpa_field_selector, "")
            logging.info("Cleared pre-filled UPI ID field")
        
        # Enter your UPI ID
        page.fill(vpa_field_selector, UPI_ID)
        logging.info(f"Entered UPI ID: {UPI_ID}")

        # Step 3: Click the "Proceed" button
        proceed_button_selector = "button.btn.btn-primary.w100"
        page.wait_for_selector(proceed_button_selector, state="visible", timeout=10000)
        page.click(proceed_button_selector)
        logging.info("Clicked Proceed button")

        # Wait for payment confirmation or further actions
        page.wait_for_load_state("networkidle", timeout=30000)
        logging.info("Payment processing initiated - check your UPI app to complete the transaction")
        
    except Exception as e:
        logging.error(f"Failed to handle Paytm UPI payment: {e}")
...
def main():
    ...
    try:
        ...
        print("Check browser window for current status")
        print("="*50 + "\n")
        time.sleep(30)

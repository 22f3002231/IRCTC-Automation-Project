...
def select_date_and_click_book_button(page):
    """Select class from tabmenu and date, then pause until Book Now is clicked manually."""
    try:
        # Debug: Check if tabmenu is present
        tabmenu_selector = "p-tabmenu"
        if page.is_visible(tabmenu_selector):
            logging.info("Tabmenu is visible on the page")
        else:
            logging.warning("Tabmenu not found on the page")

        # Select the class from the tabmenu
        class_selector = f"xpath=//p-tabmenu//li[contains(@class, 'ui-tabmenuitem')]//span[contains(., '{JOURNEY_CLASS}')]"
        logging.info(f"Attempting to locate class tab with selector: {class_selector}")
        
        class_element = page.wait_for_selector(class_selector, state="visible", timeout=TIMEOUT*1000)
        if class_element:
            logging.info(f"Class '{JOURNEY_CLASS}' tab found")
            class_element.scroll_into_view_if_needed()
            page.eval_on_selector(class_selector, "el => el.click()")  # JS click for reliability
            logging.info(f"Selected class '{JOURNEY_CLASS}' from tabmenu")
        else:
            logging.error(f"Class '{JOURNEY_CLASS}' tab not found")
        ...
        # Pause the script here, waiting for manual Book Now click
        logging.info("Script paused after class and date selection. Manually click 'Book Now' to proceed.")
        print("\n" + "="*50)
        print("PAUSED AT AVAILABILITY SELECTION")
        print("Manually click 'Book Now' in the browser")
        print("Script will resume after the click")
        print("="*50 + "\n")
        
        # Wait for the Book Now button to be clicked manually
        book_button_selector = "xpath=//button[contains(@class, 'btnDefault') and contains(., 'Book Now') and not(contains(@class, 'disable-book'))]"
        page.wait_for_selector(book_button_selector, state="visible")
        
        # Wait for navigation or some indication that Book Now was clicked
        with page.expect_navigation(timeout=0):  # No timeout, waits indefinitely
            logging.info("Waiting for manual 'Book Now' click to trigger navigation...")
        
        logging.info("'Book Now' button clicked manually, proceeding!")
        return True

    except Exception as e:
        logging.error(f"Failed in select_date_and_click_book_button: {e}")
        print("\nScript encountered an error, but browser will remain open for manual intervention")
